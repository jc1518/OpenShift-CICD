...
case $ACTION in
	...
	deploy)
		if [ -z ${VERSION} ] || [ -z ${ENVIRONMENT} ]; then
			...
			echo "Usage: ./pipeline.sh deploy <version> <environment>"
			exit 1
		fi
	...
	*)
		...
		echo 'To deploy: ./pipeline.sh deploy <version> <environment>'
		echo 'To CI/CD: ./pipeline.sh build && ./pipeline.sh deploy latest <environment>'
		exit 1		
		;;
esac		
